    ...
    <div class="container">
        ...
        foreach ($dtsiswaid as $dg);
        ?>
        <!-- <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> -->
        <form enctype="multipart/form-data" action="<?= base_url('siswa/edit_proses'); ?>" method="POST">
            <div class="form-group">
                <label for="txtnama">Nama</label>
                <!-- penting di masukkan script dibawah ini digunakan untuk penanda bahwa yang akan dirubah ini adalah id_siswa sesuai yang dirubah -->
                <!-- type="hidden" difungsikan agar input text tersebut tidak kelihatan -->
                <input value="<?php echo $dg->id_siswa; ?>" type="hidden" class="form-control" id="txtid_siswa" name="txtid_siswa" required>
            ...
            <div width="30%">
                ...
                if ($dg->foto == "kosong") {
                    ...
                    $lokasi = base_url('upload/siswa/' . $dg->foto);
                    // MENCARI LOKASI DIMANA FOTO siswa DI SIMPAN DAN NAMA FOTO siswa SUDAH TERCANTUM DI DATABASE
